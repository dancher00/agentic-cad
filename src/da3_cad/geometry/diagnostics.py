...
def write_geometry_diagnostics(
    ...
    """Write deterministic per-view PNGs, report JSON, NPZ and colored PLY."""
    ...
    point_cloud = trimesh.points.PointCloud(  # type: ignore[no-untyped-call]
        vertices=cloud.points, colors=cloud.colors
    )
    point_cloud.export(output_dir / "fused_cloud.ply")  # type: ignore[no-untyped-call]
    payload = {
