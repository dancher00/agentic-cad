...
8. Ветка `extrude` проверяет ось, восстанавливает line/circle profile и принимает
   cut только по повторяющейся замкнутой дыре в masks плюс raw 3D void evidence.
9. Ветка `revolve` сначала проверяет raw 3D radial surface. После её отказа outer
   profile можно взять из нескольких согласованных silhouettes; достроенные
   азимуты явно считаются CAD-гипотезой, а shell без видимой 3D-стенки запрещён.
