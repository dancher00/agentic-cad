...
def test_bbox_normalization_is_isotropic_and_centered() -> None:
    ...
    unit, centered, transform = normalize_bbox_isotropic(points)
...
def test_metric_cad_to_normalized_cube_to_millimeters_chain() -> None:
    ...
    contract = cad_coordinate_contract(
        ...
        backend="visual-hull-v1",
        scale=scale,
    )

    assert contract.native_kind == "metric-mm-space"
    assert contract.native_largest_extent == pytest.approx(200.0)
    assert contract.native_length_to_normalized(20.0) == pytest.approx(0.1)
    assert contract.millimeters_per_native_unit == pytest.approx(1.0)
    assert contract.millimeters_per_normalized_unit == pytest.approx(200.0)
    assert contract.normalized_length_to_millimeters(0.1) == pytest.approx(20.0)
    ...
    assert payload["metric_space"]["millimeters_per_normalized_unit"] == pytest.approx(200.0)
